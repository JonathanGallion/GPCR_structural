...
def obtainingprotdict():
	file=open('../Gprotein_associations.txt')
	Gprotdict={}
	for line in file:
		parts=line.strip('\n').split('\t')
		gprot=parts[2].split(',')
		...
		Gprotdict[parts[0]]=gprot
	print('Gprotdict',Gprotdict)
	return(Gprotdict)



def Gprotenrichment():
	Gprotdict=obtainingprotdict()
	...
	#sigposlist=['2012','2884','915','2871','1784','2875','2083','1518','1378','1603']#Skin top 10
	
	#sigposlist=['1716','2881','1484','1610','2012','1784','2734','1976','1463','2703','2871','915','2875','1378','1518','2083','1603']#Skin top 10
	if cancertype=='SKCM':
		sigposlist=['2875','2083','1518','1378','1603']
	elif cancertype=='LUAD':
		sigposlist=['2875','1603','915','1484','2764','2008','2731']
	elif cancertype=='Pancancer':
		sigposlist=['2875','2083','1518','1378','1603']
	#sigposlist=['1610','2871','2702','1645','1423','1644','915','1484','2875','1603']#Lung top 10
	#sigposlist=['2881','915','2871','1645','2145','2083','1518','1378','2875','1603']

	#sigposlist=['2871','2702','1645','1423','1644','1484','2875','1603']#Lung top 10
	mutofint=defaultdict(list)
	mutofintexp=defaultdict(list)
	refdict={}
	for gpcr in GPCR_protlist:
		if gpcr in Gprotdict:
			if '' in Gprotdict[gpcr]:
				try:
					refdict['null']+=1
				except:
					refdict['null']=1
			else:
				for i in Gprotdict[gpcr]:
					try:
						refdict[i]+=1
					except:
						refdict[i]=1
	...
	if cancertype=='Pancancer':
		path='../ImagesOlfactory/'
		for filename in glob.glob(os.path.join(path,'*/*Aug8*')):
			file=open(filename)
			...
			for line in file:
				...
				combine=mut+'#'+pat
				if alignpos in sigposlist:
					if gene in Gprotdict:
						print(Gprotdict[gene])
						if '' in Gprotdict[gene]:
							try:
								Gprotcounterdict['null']+=1
							except:
								Gprotcounterdict['null']=1
						else:
							for i in Gprotdict[gene]:
								try:
									Gprotcounterdict[i]+=1
								except:

									Gprotcounterdict[i]=1
	else:
		path='../ImagesOlfactory/%s/'%(cancertype)
		for filename in glob.glob(os.path.join(path,'*Aug8*')):
			file=open(filename)
			#file=open('../ImagesOlfactory/%s/Allmutations_Ignoreexpression_Aug8%s.txt'%(cancertype,cancertype))
			counter=0
						...
						if '' in Gprotdict[gene]:
							...
							for i in Gprotdict[gene]:
								try:
									...
									Gprotcounterdict[i]=1
	print(sum(refdict.values()))
	totalmutations=sum(Gprotcounterdict.values())
	reftotalmutations=sum(refdict.values())
	ratio=totalmutations/float(reftotalmutations)
	print(ratio,totalmutations,reftotalmutations)

	print('refdict',refdict)
	for i in refdict:
		print(i,round(refdict[i]*float(ratio),0))
	print('Gprotcounterdict',Gprotcounterdict)
	for i in Gprotcounterdict:
		print(i,Gprotcounterdict[i])
Gprotenrichment()
