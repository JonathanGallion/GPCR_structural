...
def Main():
	...
	nuclist=['A','U','C','G','-']
	nuclist=['A','U','C','G']
	cancerlist=['LUAD','SKCM','STAD','HNSC','COAD','UCEC','ACC','BLCA','BRCA','ESCA','GBM','KIRC','KIRP','LIHC','LUSC','PAAD','READ','SARC','STAD','THCA','UCEC','UCS']
	...
	###Simulating mutations for olfactory receptors
	nummutdict={'SKCM':10903,'LUAD':7392,'COAD':2165,'HNSC':2696,'LUSC':2364,'STAD':2675}
	nummut=nummutdict[cancertype]
	nucmat=CalcCodontable(cancertype,AAcodondict,mutorder)
	nucmat=nummut*nucmat
	print('nucmat',nucmat)
	nuccounter=0
	outfile=open('../ImagesOlfactory/%s/RandomMutations_olf.txt'%(cancertype),'w')
	for i in nuclist:
		nuccounter1=0
		for j in nuclist:
			if i!=j:
	...
	observedcountdict={'SKCM':5799,'LUAD':5214,'COAD':1507,'HNSC':1796,'LUSC':1632,'STAD':1933}
			...
			if protein not in badgenearray:
				...
				if position!='.' and protein in codoncountdict:
					...
					if randomcodon==proteincodon:
						print(pos,randomcodon,protein,position,proteincodon)
