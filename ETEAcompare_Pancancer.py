...
def acquiremutations(cancertype,paneamutationdict,paneamutlist):
	...
	for line in maffile:
		...
		if position!='.':
			...
			if int(position) in paneamutationdict:
				paneamutationdict[int(position)].append(EAscore)
			else:
				paneamutationdict[int(position)]=[EAscore,]
			
		eamutlist.append(EAscore)
		paneamutlist.append(EAscore)
	return(eamutationdict,eamutlist,paneamutationdict,paneamutlist)
...
def etandeamain():
	...
	paneamutationdict=defaultdict(list)
	paneamutlist=[]
	paneahypoarray=[]
	paneahyperarray=[]
	panethypoarray=[]
	...
	for cancertype in cancerlist:
		...
		eamutationdict,eamutlist,paneamutationdict,paneamutlist=acquiremutations(cancertype,paneamutationdict,paneamutlist)
		listofsigpos,lowsigpos=getlistofsigpos(cancertype)
		#print(eamutationdict)
		########Adding these EA values to the pancancer list for later
		for value in eamutlist:
			paneamutlist.append(value)
		ethyperarray=[]
		ethypoarray=[]
		earandarray=[]
		eahyperarray=[]
		...
		#####Appending these values to the pancancer lists for EA
		for value in eahyperarray:
			paneahyperarray.append(value)
		for value in eahypoarray:
			 paneahypoarray.append(value)		
		...
		#####Appending these values to the pancancer lists for ET
		for value in ethyperarray:
			panethyperarray.append(value)
		for value in ethypoarray:
			 panethypoarray.append(value)		
		...
		eatuple=[eahyperarray,eahypoarray,earandarray]
		eaplist=[]
		for i in eatuple:
			for j in eatuple:
				try:
		...
		plt.setp(plot1['medians'],          # customize median lines
				 color='Black',            # line colour
				 linewidth=2)             # line thickness
		plt.title('EA comparison of %s\n%s  %s'%(cancertype,eaplist[-3:],len(eatuple[0])))
		plt.xticks([1,2,3],['Frequent Positions','Infrequent Positions','Random Control'])
	...
	#####Making graphs for the same type of analysis for pancancer
	etplist=[]
	ettuple=[panethyperarray,panethypoarray,etallarray]
	print('panethyperarray',panethyperarray)
	print('panethypoarray',panethypoarray)
	for i in ettuple:
		for j in ettuple:
			try:
				t,p=ttest_ind(i,j)
		...
		plt.setp(plot1['medians'],          # customize median lines
				 color='Black',            # line colour
				 linewidth=2)             # line thickness
	plt.title('ET comparison of Pancancer \n%s'%(etplist[-3:]))
	plt.xticks([1,2,3],['Frequent Positions','Infrequent Positions','Random Control'])
	plt.savefig('../Paper/PotentialFigures/EAET/ET_Pancancer.png')
	plt.close()
	#plt.hist(ethyperarray,alpha=0.5)
	#plt.hist(ethypoarray,alpha=0.5)
	#plt.hist(etallarray,alpha=0.5)
	...
	eatuple=[paneahyperarray,paneahypoarray,paneamutlist,earandarray]
	eaoutfile=open('../ImagesOlfactory/EAforboxplot_Pancancer.txt','w')
	for i in eatuple:
		for j in i:
			eaoutfile.write(str(j))
			eaoutfile.write('\t')
		eaoutfile.write('\n')
	eatuple=[paneahyperarray,paneahypoarray,earandarray]
	eaplist=[]
	for i in eatuple:
		for j in eatuple:
			try:
		...
		plt.setp(plot1['medians'],          # customize median lines
				 color='Black',            # line colour
				 linewidth=2)             # line thickness
	plt.title('EA comparison of Pancancer\n%s  %s'%(eaplist[-3:],len(eatuple[0])))
	plt.xticks([1,2,3],['Frequent Positions','Infrequent Positions','Random Control'])
	
	plt.ylim(0,100)
	plt.savefig('../Paper/PotentialFigures/EAET/EA_Pancancer.png')
	plt.close()
